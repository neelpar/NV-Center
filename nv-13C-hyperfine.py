...
class NVHamiltonian():
	...
	def hyperfineHamiltonian(self,Bz):
		...
		H = self.D*(qt.tensor(self.sz*self.sz,qt.qeye(2))-(2/3)*qt.tensor(qt.qeye(3),qt.qeye(2))) + self.ge*Bz*qt.tensor(self.sz,qt.qeye(2)) + self.gc*Bz*qt.tensor(qt.qeye(3),self.Iz) + Hhf 
		eigenvals = H.eigenstates()[0]
		#print(eigenvals)
		#f0 = (eigenvals[0]+eigenvals[1])/2
		#a = np.array([(eigenvals[2]-f0),(eigenvals[3]-f0),(eigenvals[4]-f0),(eigenvals[5]-f0)])
		return eigenvals
	...
	def plotter_BvsF(self):
		...
		ax.plot(Bz,freqs[:,0],label="$m_{s}=-1$ \u2192 $m_{s}=0$, $m_{I}=-1/2$") 
		ax.plot(Bz,freqs[:,1],label="$m_{s}=-1$ \u2192 $m_{s}=0$, $m_{I}=+1/2$") 
		ax.plot(Bz,freqs[:,2],label="$m_{s}=+1$ \u2192 $m_{s}=0$, $m_{I}=+1/2$") 
		ax.plot(Bz,freqs[:,3],label="$m_{s}=+1$ \u2192 $m_{s}=0$, $m_{I}=-1/2$") 
		ax.legend()
